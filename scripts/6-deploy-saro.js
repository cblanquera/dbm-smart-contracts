...
// $ npx hardhat run scripts/1-deploy-saro.js
...
async function main() {
  ...
  console.log('Deploying SARODocument ...')
  const saro = await deploy('SARODocument', baseURI, document.address, admin.address)
  const address = await saro.getAddress();

  console.log('')
  console.log('-----------------------------------')
  console.log('SARODocument deployed to:', address)
  console.log('')
  ...
  console.log('In Document contract, grant MINTER_ROLE to SARODocument')
  console.log(` - ${network.scanner}/address/${document.address}#writeContract`)
  console.log(` - grantRole( ${getRole('MINTER_ROLE')}, ${address} )`)
  console.log('In SARODocument contract, grant MINTER_ROLE to admin (choose another wallet)')
