...
contract NCASearch {
   ...
   */
  constructor(INCADocument documentContract) {
    ...
    external view returns (INCADocument.Metadata memory) 
  {
    ...
    INCADocument.Metadata memory data = documents.documentData(documentId);
    // Serialize the document ID for comparison
    bytes32 filter = keccak256(bytes(data.ncaNumber));
